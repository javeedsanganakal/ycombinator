# YC startup role index

Source: `datasets/yc-oss-mirror/companies/all.json`
Companies classified: 6245

This index is a transparent keyword map of public YC descriptions. It is a research aid, not a claim about a company's internal hiring or organization. Review the evidence field in `yc-role-index.json` before using a match.

## Role counts

| Role | Companies |
| --- | ---: |
| founders | 352 |
| product | 1656 |
| ux-ui | 937 |
| engineering | 2857 |
| sales | 751 |
| marketing | 877 |
| monetization | 2120 |
| Unclassified | 1128 |

## Recent examples

### founders

- **Vector Legal** (Winter 2026, B2B): A premier AI-native law firm & legal operating system for Startups. [evidence: founder, startup]
- **Q2Q** (Winter 2026, B2B): We help private equity teams find acquisition targets faster [evidence: owners]
- **Polymorph** (Winter 2026, B2B): Building secure polymorphic apps [evidence: startup]
- **Pocket** (Winter 2026, Consumer): Take Notes in the Real World [evidence: company building]
- **Pax Historia** (Winter 2026, Consumer): The first AI-powered worldbuilding and gameplay platform [evidence: startup]
- **Palus Finance** (Winter 2026, Fintech): Your startup's financial advisor [evidence: startup]
- **Kita** (Winter 2026, Fintech): Underwrite borrowers around the world in minutes [evidence: founder]
- **Hlabs** (Winter 2026, Industrials): US-Made Parts for Robots [evidence: founder]
- **End Close** (Winter 2026, B2B): AI powered reconciliation for high-volume payments companies [evidence: founder]
- **Corvera** (Winter 2026, B2B): The AI sales engine for CPG brands [evidence: founder]
- **Cardinal** (Winter 2026, B2B): Revenue Agents for GTM teams [evidence: founder]
- **Cardboard** (Winter 2026, B2B): Agentic video editor [evidence: founder]

### product

- **Rote** (Winter 2027, B2B): AI-native insurance department for auto body shops [evidence: workflow]
- **Voltair** (Winter 2026, Industrials): Autonomous Drones for Earth Observation [evidence: platform]
- **Vela** (Winter 2026, B2B): AI Recruiting Coordinator for Executive Search [evidence: workflow]
- **Valgo** (Winter 2026, B2B): Insurance risk layer for physical AI [evidence: platform]
- **Valence** (Winter 2026, Fintech): Unified prediction markets trading platform [evidence: platform]
- **Terminal Use** (Winter 2026, B2B): AI-native transformation for operations-heavy companies [evidence: workflow]
- **Sila** (Winter 2026, B2B): Agent Messaging Platform [evidence: platform]
- **Sentrial** (Winter 2026, B2B): Datadog for Agent Reliability [evidence: platform]
- **RunAnywhere** (Winter 2026, B2B): The default way of running on-device AI at Scale [evidence: workflow]
- **Ruma Care** (Winter 2026, Healthcare): The operations stack for biologic infusion clinics [evidence: workflow, platform]
- **Ritivel** (Winter 2026, Healthcare): AI-native platform for Life-Sciences Documentation  [evidence: platform]
- **Rhizome AI** (Winter 2026, B2B): Agent Platform for Life Sciences [evidence: platform]

### ux-ui

- **Zatanna** (Winter 2026, B2B): Turning all software into agent-first APIs [evidence: design]
- **Visibl Semiconductors** (Winter 2026, B2B): Custom silicon for power electronics [evidence: design]
- **Valgo** (Winter 2026, B2B): Insurance risk layer for physical AI [evidence: design]
- **Synthetic Sciences** (Winter 2026, B2B): Infrastructure for Autonomous Science [evidence: design]
- **Sparkles** (Winter 2026, B2B): Make everyone on your team an engineer [evidence: design]
- **Skillsync** (Winter 2026, B2B): Notion for AI chats [evidence: design]
- **shortkit** (Winter 2026, B2B): Media infra for consumer apps [evidence: design]
- **Sentrial** (Winter 2026, B2B): Datadog for Agent Reliability [evidence: design]
- **REV1** (Winter 2026, B2B): Claude Code for Mechanical Engineers [evidence: design]
- **Reframe** (Winter 2026, B2B): AI-native, hardware procurement marketplace [evidence: design]
- **RamAIn** (Winter 2026, B2B): Automate any UI task with natural language [evidence: ui]
- **Martini** (Winter 2026, B2B): AI Video Production for Professionals [evidence: design]

### engineering

- **ZeroSettle** (Winter 2026, Fintech): Drop-in Direct Billing SDK for In-App Purchases [evidence: sdk]
- **Zatanna** (Winter 2026, B2B): Turning all software into agent-first APIs [evidence: engineering, software, cloud]
- **Wideframe** (Winter 2026, B2B): AI coworker for video editors to ship more video faster [evidence: cloud]
- **Wayco** (Winter 2026, B2B): AI operator for medlegal cases [evidence: infrastructure]
- **VOYGR** (Winter 2026, B2B): Real-world place intelligence for AI apps and agents [evidence: api]
- **Voltair** (Winter 2026, Industrials): Autonomous Drones for Earth Observation [evidence: infrastructure]
- **Visibl Semiconductors** (Winter 2026, B2B): Custom silicon for power electronics [evidence: engineering, software, infrastructure, robotics]
- **Velum Labs** (Winter 2026, B2B): The OS for data quality across any stack [evidence: engineering, infrastructure, machine learning]
- **Valgo** (Winter 2026, B2B): Insurance risk layer for physical AI [evidence: engineering]
- **Valence** (Winter 2026, Fintech): Unified prediction markets trading platform [evidence: api, infrastructure]
- **Unisson** (Winter 2026, B2B): AI agents that automate B2B software implementation [evidence: engineering, software]
- **Unifold** (Winter 2026, Fintech): Multi-chain deposit and payment infrastructure [evidence: developer, code, api, sdk, infrastructure]

### sales

- **Rote** (Winter 2027, B2B): AI-native insurance department for auto body shops [evidence: revenue]
- **VOYGR** (Winter 2026, B2B): Real-world place intelligence for AI apps and agents [evidence: gtm]
- **Velum Labs** (Winter 2026, B2B): The OS for data quality across any stack [evidence: pipeline]
- **Unisson** (Winter 2026, B2B): AI agents that automate B2B software implementation [evidence: sales]
- **Turnstone** (Winter 2026, B2B): Work with agents that already know you [evidence: gtm]
- **Signals** (Winter 2026, B2B): iMessage revenue channel for DTC brands that brings customers back [evidence: revenue]
- **Shofo** (Winter 2026, B2B): The World's Largest Video Library [evidence: pipeline]
- **Samora AI** (Winter 2026, B2B): Multilingual voice agents that outperform humans [evidence: outbound]
- **Robby** (Winter 2026, B2B): AI agents that grow revenue for home services businesses [evidence: revenue]
- **Ritivel** (Winter 2026, Healthcare): AI-native platform for Life-Sciences Documentation  [evidence: revenue]
- **Revion** (Winter 2026, B2B): Intelligence for Automotive Operations [evidence: revenue]
- **Q2Q** (Winter 2026, B2B): We help private equity teams find acquisition targets faster [evidence: outbound]

### marketing

- **Veriad** (Winter 2026, B2B): AI operating system for advertising [evidence: marketing, advertising]
- **Tepali** (Winter 2026, B2B): AI EMR for medspas & wellness clinics [evidence: marketing]
- **Sparkles** (Winter 2026, B2B): Make everyone on your team an engineer [evidence: marketing]
- **Sitefire** (Winter 2026, B2B): Marketing suite for the agentic web [evidence: marketing, growth, content]
- **Robby** (Winter 2026, B2B): AI agents that grow revenue for home services businesses [evidence: growth]
- **Remix** (Winter 2026, Consumer): Social media content auto-generated with your existing data [evidence: content, social media]
- **Oxus** (Winter 2026, B2B): AI-powered automation for internal audit workflows [evidence: social media]
- **MochaCare** (Winter 2026, B2B): Agentic Management Service for Care Organizations [evidence: growth]
- **Menza** (Winter 2026, B2B): The AI data analyst for consumer brands [evidence: brand]
- **Laurence** (Winter 2026, B2B): Quantitative research for autonomous e-commerce growth [evidence: growth, advertising]
- **General Legal** (Winter 2026, B2B): Elite AI law firm for high growth companies [evidence: growth]
- **Fixture** (Winter 2026, B2B): An AI-first CRM built for Startups [evidence: marketing]

### monetization

- **Rote** (Winter 2027, B2B): AI-native insurance department for auto body shops [evidence: revenue, insurance]
- **ZeroSettle** (Winter 2026, Fintech): Drop-in Direct Billing SDK for In-App Purchases [evidence: billing, payments, fintech]
- **VOYGR** (Winter 2026, B2B): Real-world place intelligence for AI apps and agents [evidence: commerce]
- **Voltair** (Winter 2026, Industrials): Autonomous Drones for Earth Observation [evidence: insurance]
- **Verdex** (Winter 2026, Fintech): AI Verification for Insurance [evidence: fintech, insurance]
- **Valgo** (Winter 2026, B2B): Insurance risk layer for physical AI [evidence: insurance]
- **Valence** (Winter 2026, Fintech): Unified prediction markets trading platform [evidence: fintech]
- **Unifold** (Winter 2026, Fintech): Multi-chain deposit and payment infrastructure [evidence: payment, payments, fintech]
- **SpotPay** (Winter 2026, Fintech): Stablecoin Global Bank Account [evidence: payments, fintech]
- **Sponge** (Winter 2026, B2B): Financial infrastructure for the agent economy [evidence: payments, financial]
- **Signals** (Winter 2026, B2B): iMessage revenue channel for DTC brands that brings customers back [evidence: revenue]
- **Sequence Markets** (Winter 2026, Fintech): Low-latency execution across crypto, prediction, and tokenized assets [evidence: fintech]

## Search commands

```bash
ycombinator filter --batch "Fall 2026"
ycombinator search "agent"
ycombinator search "sales"
```

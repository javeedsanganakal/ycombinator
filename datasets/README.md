...
- `yc-role-index.json` and `yc-role-index.md` map public YC descriptions to startup roles such as founders, product, UX/UI, engineering, sales, marketing, and monetization. The mapping keeps keyword evidence and is regenerated by the 24/7 agent.
- `yc-founders-index.json` and `yc-founders-index.md` join the local mirror to public founder fields on official YC profiles for records with `top_company=true`. This is a discovery category, not an independent “best founders” ranking.
